...
func (s *Store) Search(ctx context.Context, userID uuid.UUID, query string) (SearchResults, error) {
	...
	for i, u := range urlRows {
		results.URLs[i] = newURL(u.ID, u.Url, u.FeedUrl, u.IsBlockedBypass, u.Title, u.Description, u.Tags, u.CreatedAt, u.UpdatedAt)
	}
	...
	for i, f := range feedRows {
		...
		results.Feeds[i] = buildFeed(f.ID, f.FeedUrl, f.SiteUrl, f.Title, f.Description, f.ImageUrl, f.IsBlockedBypass, f.CustomName, f.LastFetchedAt, f.CreatedAt, f.UpdatedAt, nil)
	}
